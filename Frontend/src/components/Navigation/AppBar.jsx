...
const AppBar = ({ username, email }) => {
  ...
  // Hàm parse message để tách link và văn bản
  const parseMessage = (message) => {
    const linkRegex = /<a href=['"]([^'"]+)['"]>([^<]+)<\/a>/;
    const match = message.match(linkRegex);
    if (match) {
      const beforeLink = message.substring(0, match.index);
      const linkUrl = match[1];
      const linkText = match[2];
      const afterLink = message.substring(match.index + match[0].length);
      return { beforeLink, linkUrl, linkText, afterLink };
    }
    return {
      beforeLink: message,
      linkUrl: null,
                                ...
                                {card.title}
                                    ...
                                    <Box>
                                      ...
                                      <Typography
                                        ...
                                        sx={{ color: "gray" }}
                                      >
                                        {card.board_name || "No workspace"} :{" "}
                                        {card.listboard_name || "No List Board"}
                                      </Typography>
                                    </Box>
                                  </Box>
                                </Box>
                          ...
                          >
                            {(() => {
                              const {
                                ...
                                linkText,
                                afterLink,
                              } = parseMessage(firstNotification.data.message);
                              return (
                                <>
                                  {beforeLink}
                                  {linkUrl && (
                              ...
                              >
                                {(() => {
                                  const {
                                    ...
                                    linkText,
                                    afterLink,
                                  } = parseMessage(notif.data.message);
                                  return (
                                    <>
                                      {beforeLink}
                                      {linkUrl && (
